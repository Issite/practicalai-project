...
class ScriptWriter:
    ...
    def _write_lines(self, lines: list[dict]) -> None:
        ...
        :param lines: list[dict] - A list of dictionaries, each containing a "line" key with the line to write and a "print" key indicating whether to print the line to the console
        """
    ...
    def write_dialogue(self, character_name: str, dialogue: str) -> None:
        ...
        if character_name not in [character.name for character in self.characters]:
            raise ValueError(f"Character '{character_name}' not found in characters list.")
        
        out_lines = []

        if character_name == self.narrator_name:
            self.last_speaker = None
            out_lines.append({"line": f"{dialogue}", "print": True})
        elif self.last_speaker != character_name:
            self.last_speaker = character_name
            out_lines.append({"line": f"show {character_name} at bob", "print": False})
            out_lines.append({"line": f"{character_name} \"{dialogue}\"", "print": True})
        else:
            out_lines.append({"line": f"{character_name} \"{dialogue}\"", "print": True})
    ...
    @tool
    def sprite_action(self, character_name: str, sprite_id: str, action: str, description: str, value: int = None) -> None:
        """
        ...
        character = next((character for character in self.characters if character.name == character_name), None)
        if not character:
            raise ValueError(f"Character '{character_name}' not found in characters list.")
        
        if sprite_id not in character.sprites:
            raise ValueError(f"Sprite ID '{sprite_id}' not found for character '{character_name}'.")
        
        out_lines = []

        match action:
            case "show":
                out_lines.append({"line": f"show {character_name} {sprite_id} at bob", "print": False})
            case "hide":
                out_lines.append({"line": f"hide {character_name}", "print": False})
            case "xpos":
                if value is None:
                    raise ValueError("Value must be provided for 'xpos' action.")
                out_lines.append({"line": f"show {character_name} {sprite_id} at xpos {value}", "print": False})
            case _:
                raise ValueError(f"Invalid action '{action}'. Valid actions are 'show', 'hide', and 'xpos'.")
            
        out_lines.append({"line": f"{description}", "print": True})
